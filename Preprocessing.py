...
from estimate_func import estimate_annualy_income_test, est_invest
...
def Model(optuna, model, sector_name, best_params):
    nlarge = best_params['nlarge']
    miss_data_column_allowed = best_params['miss_data_column_allowed']
    miss_data_row_allowed = best_params['miss_data_row_allowed']
...
def predict_invest(model, sector_name, best_params_model, best_params_estim):
    ...
    y_test = df_final['Stock_Price']

    x_train = preprocessing.normalize(df_final.drop(['Stock_Price', 'Ticker'], axis=1).to_numpy())
    x_test = preprocessing.normalize(df_final.drop(['Stock_Price', 'Ticker'], axis=1).to_numpy())

    model, result = model(None, x_train, y_test, x_test, y_test, train=False, best_params=best_params_model)


    bear_inv = best_params_estim['bear_inv']
    price_discount = best_params_estim['price_discount']

    return est_invest(model, df_final, y_test, price_discount=price_discount, bear_inv=bear_inv, sector_name=sector_name)